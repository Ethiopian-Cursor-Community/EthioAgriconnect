...
const RateFarm = () => {
  ...
  useEffect(() => {
    ...
    const matchedOrder = deliveredOrders.find((order) => order.id === orderId);
    if (matchedOrder) {
      setSelectedOrder(matchedOrder);
    }
  }, [deliveredOrders, searchParams, selectedOrder]);
  ...
  const getOrderDisplayName = (order: Order) => {
    ...
    return order.productName || 'Delivered order';
  };
  ...
  const fetchDeliveredOrders = async () => {
    try {
      ...
      setDeliveredOrders(
        ordersData.filter((order) => isDeliveredOrder(order) && !order.hasConsumerRating)
      );
    } catch (error) {
  ...
  const handleSubmitRating = async () => {
    ...
    try {
      await addDoc(collection(db, 'ratings'), {
        ...
        productName: getOrderDisplayName(selectedOrder),
        stars: rating,
        comment,
        ...
        {deliveredOrders.length === 0 ? (
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {deliveredOrders.map((order) => (
              ...
              >
                <h3 className="text-lg font-semibold text-gray-900 mb-2">{getOrderDisplayName(order)}</h3>
        ...
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center p-4 z-50">
          <div className="bg-white rounded-xl p-8 max-w-md w-full">
            ...
            <p className="text-gray-600 mb-6">
              {getOrderDisplayName(selectedOrder)} from {selectedOrder.farmerName}
            </p>
