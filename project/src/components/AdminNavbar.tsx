...
import { Sprout, LogOut, User, LayoutDashboard, Users, Package, ShoppingBag, DollarSign, Star, Megaphone, FileText, Settings, Truck, Sparkles } from 'lucide-react';
...
const AdminNavbar = () => {
  ...
  const adminLinks = [
    ...
    { to: '/admin/delivery-assignment', label: 'Deliveries', icon: Truck },
    { to: '/admin/prices', label: 'Prices', icon: DollarSign },
    { to: '/admin/reviews', label: 'Reviews', icon: Star },
    { to: '/admin/announcements', label: 'Announcements', icon: Megaphone },
    { to: '/admin/reports', label: 'Reports', icon: FileText },
    { to: '/admin/ai-showcase', label: 'AI Future', icon: Sparkles },
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex items-center justify-between h-16">
          <Link to="/admin/dashboard" className="flex items-center gap-2">
            ...
            <span className="text-2xl font-bold text-green-800">EthioAgriConnect Admin</span>
          </Link>
            ...
            {adminLinks.map((link) => {
              ...
              return (
                <Link
                  key={link.to}
                  to={link.to}
                  className="flex items-center gap-0.5 px-1.5 py-2 text-gray-700 hover:text-green-600 hover:bg-green-50 rounded-lg font-medium transition text-xs"
                >
